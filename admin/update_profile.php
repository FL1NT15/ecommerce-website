...
// Check if the admin is updating their own account or another's
$is_own_account = ($admin_to_update === $admin_id);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Update role logic (for updating other admins' roles or their own role if allowed)
    if (!$is_own_account || $current_profile['role_id'] == 1) { 
    // If it's not the admin's own account, or they are a super admin, allow role change
...
} elseif ($is_own_account) {
    $message[] = 'You cannot change your own role!'; // Retain this message if you want to prevent self-role updates
}
...
<!-- Update Role -->
<?php if (!$is_own_account): // Only show role selection if not editing own account ?>
...
<?php else: ?>
    <input type="hidden" name="role_id" value="<?= htmlspecialchars($current_profile['role_id']); ?>">
<?php endif; ?>
